 ...
 if (isset($_GET['id'])) {
 	...
 	$pdf->Cell(5,3,':',0,0);
 	$x =$pdf->getX();
 	$pdf->dynamic_cell(40,3,$x,$cu_details['address'],0,0);
 	$pdf->Ln();
 	...
 	$sql1 = "SELECT note FROM `ticket` WHERE `cus_contact`='".$phone."' AND `order_id`='".$order_id."' ORDER BY id DESC LIMIT 1";
 	...
 	if (isset($order_id)) {
 		...
 		$x = $pdf->getX();
 		$pdf->cell(25,3,'Product Name',0,0,'L');
 		...
 		$pdf->SetFont('Arial','',8);
			...
			//summary
 			...
 			$pdf->Cell(20,2,'',0,0);
 			$pdf->Cell(20,2,'Total Payable',0,0,'L');
 			$pdf->Cell(5,2,'=',0,0);
 			$pdf->Cell(10,2,(round($total_price+$total_vat+$total_sd)-$discount),0,1,'L');//end of line
 			$pdf->Ln();
