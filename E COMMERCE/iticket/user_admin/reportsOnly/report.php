...
include '../../db.php';
  ...
  $results3=mysql_query("SELECT user_name,concern FROM users WHERE id=".$pad_user);
...
while($row=mysql_fetch_array($result1)) { ?>
		...
		Wellcome <?php print $row222[0];?> [<?php print $row222[1]; ?>]
		
		</td>
      </tr>
      <tr>
        <td width="115"><label class="glabel">Ticket Type </label></td>
        <td width="151"><select name="type" class="form-consex" id="type" >
            <!--<option value="">-Select Type-</option>-->
            <? include '../../db.php';
					$result1 = mysql_query("select * FROM ticket_type where type_name = '$row222[1]'");
while($row=mysql_fetch_array($result1)) { ?>
...
while($row=mysql_fetch_array($result1)) { ?>
        ...
        <td><input type="submit" class="koblaex" name="Submit2" value="Download to Excel" onclick="window.location.assign('phpexcel/report_type_status.php?idate='+document.getElementById('idate').value+'&edate='+document.getElementById('edate').value+'&status='+document.getElementById('status').value+'&type='+document.getElementById('type').value)"/></td>
        <td>
      ...
      <tr>
        ...
        <td><input type="text" name="skeyword" id="skeyword" class="gtext" style="width:125px;" />
            <img src="search.png" width="20" height="20" style="cursor:pointer; background-color:#FFFFFF; vertical-align:bottom; border-top-left-radius:3px; border-top-right-radius:3px; border-bottom-left-radius:3px; border-bottom-right-radius:3px;" onclick="reportJAVAC(document.getElementById('idate').value,document.getElementById('edate').value,document.getElementById('sby').value,document.getElementById('skeyword').value,'<?php print $row222[1]; ?>')" /></td>
