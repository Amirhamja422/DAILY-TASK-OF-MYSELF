...
$filename="iTicket_Report_PRAN-RFL_CRD".date("Y-m-d");							//File Name to be Exported
$topgap=2;										//Line Gap in Excel File Top (Must Greater than 1)
$color='F28A8C';
$color2='FFFFFF';


$sql = "SELECT `id` as 'ID', `ticket_type` as 'Type', `from` as 'Raised', (select user_name from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `cus_ac` as 'Account ID', `cus_product` as 'Product Details', `cus_amount` as 'Address', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE $q3  like '%$q4%' AND date >='$q1 00:00:00'  AND date <='$q2 23:59:59' and ticket_type = '$q5'"; 
