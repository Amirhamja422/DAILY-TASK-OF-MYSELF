...
$DB_DBName = "pranrfl";				//your MySQL Database Name 
$DB_TBLName = "crm";				//your MySQL Table Name 
...
$sql = "SELECT id as 'ID', brand as 'Group', name as 'Full Name',contact as 'Phone', address as 'Address', product as 'Product', p_model as 'Product Model', pur_date as 'Purchase Date', a_person as 'Assigned Person', a_date as 'Assigned Date',report_type as 'Report Type', problem_status as 'Current Status',detail as 'Details', father as 'Father Name', dob as 'Date of Birth', sex as 'Sex' FROM crm where ".$q3." like '%".$q4."%' && report_type like '%".$q5."%'  && `a_date` >= STR_TO_DATE('".$q1."','%Y-%m-%d')  && `a_date` <= STR_TO_DATE('".$q2."','%Y-%m-%d')  ORDER BY `crm`.`a_date`  DESC";
...
header("Content-Disposition: attachment; filename=Call-Center-Report__Pran-RFL_Group.$file_ending");
