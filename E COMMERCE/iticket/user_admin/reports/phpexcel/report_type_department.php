<?php
/*Get Data*/
echo $id  = $_GET['idate'];  // Date Initial
echo $ed  = $_GET['edate'];  // Date End
echo $st  = $_GET['department'];
?>
...
$color2='FFFFFF';
 ...
 $sql = "SELECT `id` as 'ID', `ticket_type` as 'TYPE', `agent` as 'Raised', (select group_name from user_group where id = `ticket`.`group`) as 'Department', `cus_contact` as 'Phone', `cus_name` as 'Name', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE `group`='".$st."' AND date >='".$id." 00:00:00'  AND date <='".$ed." 23:59:59'";
 echo "s";
