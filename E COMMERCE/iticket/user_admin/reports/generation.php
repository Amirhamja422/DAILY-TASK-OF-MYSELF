...
$results=mysql_query("SELECT `id` as 'ID', `ticket_type` as 'TYPE', `agent` as 'Raised', (select user_name from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `cus_ac` as 'Account ID', `cus_product` as 'Product Details', `cus_amount` as 'Address', `status` as 'Last Status', `date` as 'Create Date', `priority` as 'Priority' FROM `ticket` WHERE ".$q3." like '%".$q4."%' AND date >='".$q1." 00:00:00'  AND date <='".$q2." 23:59:59'");
...
<a href="phpexcel/report.php?q1=<?php print $q1; ?>&q2=<?php print $q2; ?>&q3=<?php print $q3; ?>&q4=<?php print $q4; ?>&q5=">
...
print "<thead align=\"center\"><tr><th>ID</th><th>TYPE</th><th>Raised</th><th>Assigned</th><th>Contact</th><th>Name</th><th>Last Status</th><th>Carete Date</th><th>Priority</th></tr></thead>";
...
{   // loop Start
  ...
  {
     print "<tr>";
print "<td>".$data_array[0]."</td><td>".$data_array[1]."</td><td>".$data_array[2]."</td><td>".$data_array[3]."</td><td>".$data_array[4]."</td><td>".$data_array[5]."</td><td>".$data_array[9]."</td><td>".$data_array[10]."</td><td>".$data_array[11]."</td>";
print "</tr>";
