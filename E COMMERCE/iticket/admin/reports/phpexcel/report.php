...
$sql = "SELECT `id` as 'ID', `ticket_type` as 'Type', `agent` as 'Raised', (select user_name from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE $q3  like '%$q4%' AND date >='$q1 00:00:00'  AND date <='$q2 23:59:59' "; 
