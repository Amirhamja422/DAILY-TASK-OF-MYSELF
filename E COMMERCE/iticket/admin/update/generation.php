...
$results=mysql_query("SELECT `id` as 'ID', `group` as 'TYPE', `agent` as 'Raised', (select user_name from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `branch_name` as 'branch_name',   `date` as 'Create Date' ,`status` as 'STATUS',`another_name` as 'ANOTHER',`cus_ac` as `pro`,`order_id` as `order_id` FROM `ticket` WHERE ".$q3." like '%".$q4."%' AND date >='".$q1." 00:00:00'  AND date <='".$q2." 23:59:59' ORDER BY `ticket`.`id` DESC");
...
print "<thead align=\"center\"><tr><th>ID</th><th>Brand</th><th>Raised</th><th>Assigned</th><th>Contact</th><th>Name</th><th>Branch Name</th><th>Create Date</th><th>Status</th><th title=\"UPDATE\"><img src=\"update.png\" width=\"20\" height=\"20\"></th></tr></thead>";
...
{   // loop Start
  ...
  {
   print $tr;
   print "<td>".$data_array[0]."</td><td>".$data_array[1]."</td><td>".$data_array[2]."</td><td>".$data_array[3]."</td><td>".$data_array[4]."</td><td>".$data_array[5]."</td><td>".$data_array[6]."</td><td>".$data_array[7]."</td><td>".$data_array[8]."</td><td><a onclick=\"smcollege('".$data_array[0]."','".$data_array[11]."');\"><img src=\"edit.png\" width=\"15\" height=\"15\" style=\"cursor:pointer;\" title=\"Update Ticket $data_array[0]\"></a></td>";
   print "</tr>";
