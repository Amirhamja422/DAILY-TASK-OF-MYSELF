...
if($group == 'all'){
  $sql = "SELECT `id` as 'ID', `group` as 'Brand', `agent` as 'Raised', (select user_id from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE `group` ='$group' AND date >='$id 00:00:00'  AND date <='$ed 23:59:59'";
}else{
