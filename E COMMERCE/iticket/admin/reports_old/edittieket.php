	...
	<table align="center" style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px;">
		...
		<tr>
			...
			<tr>
				...
				<td>
					<select name="to" class="form-consex" id="to" required>
						<?php
						$result1 = mysql_query("select id,user_name FROM users where `user_group_id`='".$data_array[4]."' ORDER BY user_id DESC");
						while($row=mysql_fetch_array($result1)) { 
			...
			<!-- <tr>
				...
				<td>:</td>
				<td> --><input name="account_id" type="hidden" class="form-consex" readonly id="account_id" value="<?php print $data_array[8];?>"/><!-- </td>

				<td>Card/CIF</td>
				<td>:</td>
				<td> --><input name="amount" type="hidden" class="form-consex" id="amount" readonly value="<?php print $data_array[10];?>"/><!-- </td>
			</tr> -->
			...
			<tr>
				...
				<td>
					...
					<td>:</td>
					<td><input name="cc" type="text" class="form-consex" id="cc" value="<?php print $data_array[16];?>" placeholder="More Recipient(s)"/></td>
				</tr>
				<tr>
...
if(isset($_POST['ishir']))   //ishir Check
{
	...
	Ticket Updated Successfully.</font></div>";		
			...
			$results=mysql_query("UPDATE `ticket_dev`.`ticket` SET `ticket_type` = '".$_POST['type']."', `assignd` = '".$_POST['to']."', `group` = '".$_POST[	
				'group']."', `cus_contact` = '".$_POST['cus_phone']."',   				
				`cus_name` = '".$_POST['cus_name']."', `cus_ac` = '".$_POST['account_id']."', `cus_product` = '".$_POST['product']."', `cus_amount` = '".	
				$_POST['amount']."', `details` = '".$_POST['editor1']."', `sub_group` = '".$_POST['sub_group']."', `stat` = '1' ,`status` = '".$_POST['status']."', `superiors` = '".$_POST['cc']."' WHERE `ticket`.`id` = $Global_id");
