...
$filename="iTicket_Report(Ticket-Assigned)_PRAN-RFL_CRD".date("Y-m-d");							//File Name to be Exported
$topgap=2;										//Line Gap in Excel File Top (Must Greater than 1)
$color='F28A8C';
$color2='FFFFFF';

 
 $sql = "SELECT `id` as 'ID', `ticket_type` as 'TYPE', `from` as 'Raised', (select user_id from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `cus_ac` as 'Account ID', `cus_product` as 'Product Details', `cus_amount` as 'Address', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE (assignd='".$type."' or FIND_IN_SET(".$type.",`superiors`)) AND date >='".$id." 00:00:00'  AND date <='".$ed." 23:59:59'  ";
