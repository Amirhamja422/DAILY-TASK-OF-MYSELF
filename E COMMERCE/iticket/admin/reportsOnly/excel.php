...
$sql = "SELECT `id` as 'ID', `ticket_type` as 'TYPE', `from` as 'Raised BY', (select user_id from users where id = `ticket`.`assignd`) as 'Assigned TO', `cus_contact` as 'CONTACT', `cus_name` as 'Name', `cus_ac` as 'Account ID', `cus_product` as 'Product Details', `cus_amount` as 'Disagreement Amount ', `status` as 'Last Status', `date` as 'Create Date', `priority` as 'Priority' FROM `ticket` WHERE $q3  like '%$q4%' AND date >='$q1'  AND date <='$q2' ";    
...
header("Content-Disposition: attachment; filename=iTracker-Report-SINGER-Bangladesh.$file_ending");
