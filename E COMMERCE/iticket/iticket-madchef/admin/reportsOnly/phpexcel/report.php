...
$filename="iTicket_Report_IFIC".date("Y-m-d");							//File Name to be Exported
$topgap=2;										//Line Gap in Excel File Top (Must Greater than 1)
$color='F28A8C';
$color2='FFFFFF';


$sql = "SELECT `id` as 'ID', (select type_name from ticket_type where id = `ticket`.`ticket_type`) as 'Service', (select sub_group_name from sub_group where id = `ticket`.`sub_group`) as 'Issue',`from` as 'Raised', (select user_name from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `cus_ac` as 'Order No', `cus_product` as 'Email', `cus_amount` as 'Title', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE $q3  like '%$q4%' AND date >='$q1 00:00:00'  AND date <='$q2 23:59:59' and `group` = '$q5'"; 
