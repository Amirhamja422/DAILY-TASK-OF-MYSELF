...
$sql = "SELECT * FROM crm where ".$q3." like '%".$q4."%' && report_type like '%".$q5."%'  && `a_date` >= STR_TO_DATE('".$q1."','%Y-%m-%d')  && `a_date` <= STR_TO_DATE('".$q2."','%Y-%m-%d')  ORDER BY `crm`.`a_date`  DESC";
...
$title = "CRM Report ---  PRAN-RFL Group (Customer Relationship Department)";//"Dump For Table $DB_TBLName from Database $DB_DBName on $now_date";
/*
