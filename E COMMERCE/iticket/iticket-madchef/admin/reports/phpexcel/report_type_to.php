...
$color2='FFFFFF';
 ...
 //$sql = "SELECT `id` as 'ID', (select type_name from ticket_type where id = `ticket`.`ticket_type`) as 'TYPE', `from` as 'Raised', (select user_id from users where id = `ticket`.`assignd`) as 'Assigned', `cus_contact` as 'Phone', `cus_name` as 'Name', `cus_ac` as 'Account ID', `cus_amount` as 'Card Number',`cus_product` as 'Product Details', `status` as 'Last Status', `date` as 'Create Date', `details` as 'Details',(SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',(SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE (assignd='".$type."' or FIND_IN_SET(".$type.",`superiors`)) AND date >='".$id." 00:00:00'  AND date <='".$ed." 23:59:59'  ";
 $sql = "SELECT `id` as 'ID', (select type_name from ticket_type where id = `ticket`.`ticket_type`) as 'Service', (select sub_group_name from sub_group where id = `ticket`.`sub_group`) as 'Issue',`from` as 'Raised', (select user_id from users where id = `ticket`.`assignd`) as 'Assigned', `cus_name` as 'Name', `cus_contact` as 'Phone', `cus_ac` as 'Account No', `cus_amount` as 'Card/CIF', `details` as 'Details', (SELECT details FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail',`cus_product` as 'Email', `status` as 'Last Status', `date` as 'Create Date', (SELECT date FROM `history` where id = `ticket`.`id` ORDER BY `history`.`date` DESC limit 1) as 'Last Detail Date' FROM `ticket` WHERE (assignd='".$type."') AND date >='".$id." 00:00:00'  AND date <='".$ed." 23:59:59'  ";
...
$objPHPExcel->getProperties()->setLastModifiedBy("iHelpbd");
$objPHPExcel->getProperties()->setTitle("iTicket Report");
$objPHPExcel->getProperties()->setSubject("iTicket Report");
