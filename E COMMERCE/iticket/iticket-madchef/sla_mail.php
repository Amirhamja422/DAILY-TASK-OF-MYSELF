...
$sql = mysql_query("SELECT * FROM `ticket_dev`.`ticket` WHERE `hour_time`!='0' AND `status` != 'Closed' ORDER BY `id` DESC");
...
while ($row = mysql_fetch_assoc($sql)) {
  ...
  if (($create_time > "10:00:00") && ($create_time < "17:59:59")) {
    ...
    echo "SLA: ".$sla = $row['hour_time'];echo "<br>";
    ...
    echo "------Calculation------<br>";
     ...
     echo "Total Gap: ".$calculation = ($tot_hour - ($start_ga + $today_end_gap));echo "<br>"; 
      ...
      if (($dayname != "Fri") && ($dayname != "Sat")) {
       ...
       echo "Total Hour By Day: ".$tot_hour = (($day_count * 8) * 3600);echo "<br>"; 
       // echo "Start Gap: ".$start_gap = (strtotime($create) - strtotime($unique_start));echo "<br>";
       echo "Today End Gap: ".$today_end_gap = (strtotime($today_end) - strtotime(date("Y-m-d H:i:s")));echo "<br>"; 
