...
function showSend(to,msg,mask,pass)
{
...
document.getElementById("suc_res").src = "https://vas.banglalinkgsm.com/sendSMS/sendSMS?msisdn="+to+"&message="+msg+"&userID=PRANRFL&passwd="+pass+"&sender="+mask;
...
<?php
include 'db.php';
$pass_res = mysql_query("SELECT `password` FROM `sms_api`");
...
<br>				
<button class="btn btn-primary btn-sm" onclick="showSend(document.getElementById('displaymnumbers').value,document.getElementById('smsbody').value,document.getElementById('smsmask').value,'<?php print $SMS_PASS[0];?>');" style="float:left;"> <span class="glyphicon glyphicon-send" aria-hidden="true"></span> Send SMS</button>
