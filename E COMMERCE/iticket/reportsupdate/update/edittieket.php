...
<td>Customer ID</td>
<td>:</td>
<td><input name="account_id" type="text" class="form-consex" id="account_id" value="<?php print $data_array[7];?>"/></td>

<td>Address</td>
<td>:</td>
<td><input name="amount" type="text" class="form-consex" id="amount" value="<?php print $data_array[9];?>"/></td>
</tr>
...
<td>Product</td>
<td>:</td>
<td><input name="product" type="text" class="form-consex" id="product" value="<?php print $data_array[8];?>"/></td>
</tr>
...
if(isset($_POST['ishir']))   //ishir Check
		{
		$results=mysql_query("UPDATE `ticket`.`ticket` SET `ticket_type` = '".$_POST['type']."', `assignd` = '".$_POST['to']."', `group` = '".$_POST[	
		'group']."', `cus_contact` = '".$_POST['cus_phone']."',   				
		`cus_name` = '".$_POST['cus_name']."', `cus_ac` = '".$_POST['account_id']."', `cus_product` = '".$_POST['product']."', `cus_amount` = '".	
		$_POST['amount']."', `details` = '".$_POST['editor1']."', `superiors` = CONCAT((select superior_id from users where id=".$_POST['to']."),\"".	
		...
		'group']."', `cus_contact` = '".$_POST['cus_phone']."',   				
		`cus_name` = '".$_POST['cus_name']."', `cus_ac` = '".$_POST['account_id']."', `cus_product` = '".$_POST['product']."', `cus_amount` = '".	
		$_POST['amount']."', `details` = '".$_POST['editor1']."', `superiors` = '".substr($_POST['cc'], 1)."' WHERE `ticket`.`id` = $Global_id");
		print "<div align=\"center\"><font face=\"Verdana, Arial, Helvetica, sans-serif\" color=\"#003300\">
		Ticket Updated Successfully.</font></div>";
		}
}
