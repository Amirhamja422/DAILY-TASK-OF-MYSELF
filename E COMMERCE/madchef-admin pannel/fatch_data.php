...
if (isset($_POST['subcategory_name']) && $_POST['subcategory_name'] !=null) 
{
 $subcategory_name = $_POST['subcategory_name'];
 $chksql = "SELECT * FROM `subcategory` WHERE `subcategory` ='".$_POST['subcategory_name']."'";
// echo $chksql;
 $count = mysqli_num_rows(mysqli_query($con,$chksql));
 // echo $count;
 if ($count > 0) {
 ...
 $sql=$con->query("INSERT INTO `subcategory`(`subcategory`) VALUES ('$subcategory_name')");
 if ($sql) 
  print "status Added Successfully";
}
}
elseif (isset($_POST['subcategory_name']) && $_POST['subcategory_name']=="") {
  echo "Please Enter Valid Input";
}
...
if ((isset($_POST['sub_brand_name']) && $_POST['sub_brand_name']!=null) && isset($_POST['sub_branch_name'])  && isset($_POST['newp_category_name']) && isset($_POST['sub_details'])  && isset($_POST['sp_sub_category_name'])  && isset($_POST['sub_product_name']) && isset($_POST['sub_size'])  && isset($_POST['sub_price'])  && isset($_POST['sub_vat'])  && isset($_POST['sub_sd'])  && isset($_POST['sub_type'])  && isset($_POST['sub_discount']) && isset($_POST['sub_status'])) 
{
  ...
  if (!empty($_FILES)) {
    ...
    $allwoed_extention = array('jpg','JPG','png');
  ...
  if ($pass == 0) {
    $sql=$con->query("INSERT INTO `restaurant`.`su_product` (`name`, `deatails`, `image`, `size`, `price`, `vat`, `sd`, `type`, `dicount`, `status`, `brand`, `branch`, `category`, `subcategory`) VALUES ('$sub_product_name','$sub_details', '$sub_newfilename','$sub_size', '$sub_price', '$sub_vat', '$sub_sd', '$sub_type', '$sub_discount', '1', '$sub_brand_name', '$sub_branch_name', '$newp_category_name', '$sp_sub_category_name')");
    if($sql){
...
if ((isset($_POST['pro_brand_name']) && $_POST['pro_brand_name']!=null) && isset($_POST['pro_branch_name']) && isset($_POST['pro_details'])&& isset($_POST['pro_category_name'])&& isset($_POST['pro_product_name'])&& isset($_POST['pro_size']) && isset($_POST['pro_price'])&& isset($_POST['pro_vat']) && isset($_POST['pro_sd'])&& isset($_POST['pro_type'])&& isset($_POST['pro_discount']) && isset($_POST['pro_status'])) {
  ...
  if (!empty($_FILES)){
    ...
    $allwoed_extention = array('jpg', 'JPG', 'png' ,'PNG' ,'jpeg' ,'JPEG');
    if(in_array($ext, $allwoed_extention)){
  ...
  if ($pass == 0) {
    $result=$con->query("INSERT INTO `restaurant`.`product` (`name`,`deatails`, `image`, `size`, `price`, `vat`, `sd`, `type`, `dicount`, `status`, `brand`, `branch`, `category`, `subcategory`,`adons_status`) VALUES ('$product_name','$pro_details', '$newfilename', '$size', '$price', '$vat', '$sd', '$type', '$discount', '1', '$brand_name', '$branch_name', '$category_name', '$sub_category_name','$adons_status')");
    if($result){
      echo 'Product Added Successfully...';
    } else{
...
if ($pass == 0) {
 $sql=$con->query("INSERT INTO `restaurant`.`adons_product` (`name`, `deatails`, `image`, `size`, `price`, `vat`, `sd`, `type`, `dicount`, `status`, `brand`, `branch`, `category`, `subcategory`, `created_at`, `updated_at`) VALUES ('$adons_product_name','$adons_details', '$adons_newfilename','$adons_size', '$adons_price', '$adons_vat', '$sub_sd', '$adons_type', '$adons_discount', '1', '$adons_brand_name', '$adons_branch_name', '$adons_category_name', '$adons_sub_category_name')");
 if($sql){
  echo 'Product Added Successfully...';
} else{
  echo 'Product Not Added....';
