  ...
  if (isset($_POST['post_id'])) {
    ...
    if (!empty($_FILES)) 
    {
      ...
      if(in_array($ext, $allwoed_extention)){
        if ($_FILES['attachment']['size'] < 10485760) {
          ...
          $sql = "UPDATE `su_product` SET `brand`='$brand_update',`branch`='$branch_update',`deatails`='$deatails_update',`name`='$name_update',`category`='$category_update',`price`='$price_update',`vat`='$vat_update',`sd`='$sd_update',`dicount`='$dicount_update',`image`='$newfilename' WHERE `id`='$id_update'";
    ...
    $sql = "UPDATE `su_product` SET `brand`='$brand_update',`branch`='$branch_update',`deatails`='$deatails_update',`name`='$name_update',`category`='$category_update',`price`='$price_update',`vat`='$vat_update',
   ...
   <div class="form-group">
    ...
    <div class="col-sm-9">
     <!--  <input type="text" class="form-control"name="category_update" id="category_update" type="text" value="<?php echo $assoc['category']; ?>" placeholder="Enter Youser Name" required="required"> -->
       ...
       <select name="category_update" class="form-control input js-example-basic-single" style="width:323px;"  id="category_update">

          <option selected ><?php echo $assoc['subcategory']; ?></option>
          <?php
          include_once 'database.php';
        ...
        while($row = mysqli_fetch_assoc($category)){?>
          <option
           ...
           if ($row['subcategory'] == $row['subcategory']) {
            ...
            ?>
           value="<?php echo $row['subcategory'];?>"><?php echo $row['subcategory'];?></option>
          <?php
