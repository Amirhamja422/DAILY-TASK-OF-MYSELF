...
$end = date('Y-m-d') . " 23:59:59"; 
 ...
 $total_result = mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."'  AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."') GROUP BY `order_id`");
 $total=mysqli_num_rows($total_result);


 $completed_result = mysqli_query($con,"SELECT *  FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='Completed') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' GROUP BY `order_id`");
 $completed=mysqli_num_rows($completed_result);


 $received_result = mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='Received') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' GROUP BY `order_id` ");
$received=mysqli_num_rows($received_result);


 $pending_result  = mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='Pending') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' GROUP BY `order_id` ");
 $panding=mysqli_num_rows($pending_result);

 $cooking = mysqli_num_rows(mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='Cooking') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."'  GROUP BY `order_id`"));

 $shipped = mysqli_num_rows(mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='Shipped') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."'  GROUP BY `order_id`"));

 $new = mysqli_num_rows(mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='new') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' GROUP BY `order_id`"));

  $cancelled = mysqli_num_rows(mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE (`brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."') AND (`order_status`='cancelled') AND `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' GROUP BY `order_id`"));

  $sql = mysqli_query($con,"SELECT * FROM `restaurant`.`order` WHERE  `cart_time` >= '".$start."' AND `cart_time` <= '".$end."' AND `brand`='".$_SESSION['brand']."' AND `branch`='".$_SESSION['branch']."'  GROUP BY `order_id`  ORDER BY `id` DESC LIMIT 20");
?>
  ...
  <div class="row w-100" style="margin-top: -13px; height: 291px; margin-left: 47px;">
    <?php  if($_SESSION['previlege'] == 3){?>
      <div class="col-md-3" style="background-image: url('image/total.png'); background-repeat: no-repeat; background-size: 65% 75%;color: white;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Total Order <span class="span" ><?php echo  $total; ?></span> </h4></div>
      </div>
      <div class="col-md-3" style="background-image: url('image/new.png'); background-repeat: no-repeat; background-size: 65% 75%; color: black;">
        <div class="text-center mt-2"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Received <span class="span"><?php echo $received; ?></span>  </h4></div>
      </div>
      ...
      <div class="col-md-3" style="background-image: url('image/Resolution_SLA_failed.png'); background-repeat: no-repeat; background-size: 65% 75%; color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Pending <span class="span"><?php echo $panding; ?></span> </h4></div>
      </div>
      ...
      <div class="col-md-3" style="background-image: url('image/cooking.png'); background-repeat: no-repeat; background-size: 65% 75%;color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Cooking  <span class="span"><?php echo $cooking; ?></span> </h4></div>
      </div>
      ...
      <div class="col-md-3" style="background-image: url('image/shipped.png'); background-repeat: no-repeat; background-size: 65% 75%; color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;"> Shipped <span class="span"><?php echo $shipped; ?></span> </h4></div>
      </div>
      ...
      <div class="col-md-3" style="background-image: url('image/completed.png'); background-repeat: no-repeat; background-size: 65% 75%;color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Completed <span class="span"><?php echo  $completed; ?></span> </h4></div>
      </div>
      ...
      <div class="col-md-3" style="background-image: url('image/cancel.png'); background-repeat: no-repeat; background-size: 65% 75%; color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">Cancelled <span class="span"><?php echo $cancelled; ?></span></h4></div>
      </div>



     <div class="col-md-3" style="background-image: url('image/cooking.png'); background-repeat: no-repeat; background-size: 65% 75%; color: black;">
        <div class="text-center mt-3"><h4 class="h4" style="margin-top:60px;margin-right:120px;">New <span class="span"><?php echo $new; ?></span></h4></div>
      </div>
          ...
          <tbody align="center">
            ...
            while($ticket = mysqli_fetch_assoc($sql)){
              ...
              <tr>
                ...
                <td>
                  <select class="form-control" style="height: 32px; width: 160px;" <?php echo $disabled;?> id="change_status<?php echo $ticket['order_id'];?>" onchange="modify('<?php echo $ticket['order_id'];?>')">
